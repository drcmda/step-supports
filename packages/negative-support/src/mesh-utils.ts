/**
 * Mesh utility functions: bounding box, signed volume, z-translation,
 * inflation (vertex-normal offset), and mesh repair.
 */

import type { ParsedMesh, BBox } from './types';
 ...
 * At small margins (0.1-0.5mm) this is effectively identical (~0.3% volume delta).
 */
...
export function repairMesh(mesh: ParsedMesh): ParsedMesh {
  ...
  for (let i = 0; i < numTri; i++) {
    ...
    if (a === b || b === c || a === c) { keep[i] = false; continue; }
    ...
    const ny = e1z * e2x - e1x * e2z;
    const nz = e1x * e2y - e1y * e2x;
    if (nx * nx + ny * ny + nz * nz < 1e-16) { keep[i] = false; continue; }

    const sorted = [a, b, c].sort((x, y) => x - y);
    const faceKey = `${sorted[0]},${sorted[1]},${sorted[2]}`;
    if (faceSet.has(faceKey)) { keep[i] = false; continue; }
    faceSet.add(faceKey);
  }
  ...
  for (let i = 0; i < numTri; i++) {
    if (keep[i]) keptFaces.push(f[i * 3], f[i * 3 + 1], f[i * 3 + 2]);
  }
  ...
  return { vertices: new Float32Array(newVerts), faces: newFaces };
}
