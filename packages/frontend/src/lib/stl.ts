...
export function parseSTL(buffer: ArrayBuffer): ParsedMesh {
  ...
  // Check if ASCII: starts with "solid" and contains "facet"
  ...
  if (header.startsWith('solid') && buffer.byteLength > 84) {
    // Could be ASCII or binary with "solid" header — check if binary tri count makes sense
...
/** Export a mesh to binary STL format. Returns an ArrayBuffer. */
